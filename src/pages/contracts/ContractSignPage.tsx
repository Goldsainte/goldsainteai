...
export default function ContractSignPage() {
  ...
  // Plain-English summary — generated once per contract by Goldsainte AI,
  // then cached on the contract row; every later viewer reads the cache.
  useEffect(() => {
    if (!contract || summaryRequested) return;
    if (contract.source_type === "uploaded") return;
    if (contract.plain_summary) {
      setPlainSummary(contract.plain_summary);
      return;
    }
    ...
    (async () => {
      try {
        const { data, error } = await supabase.functions.invoke("ai-proposal-polish", {
          body: { mode: "contract_summary", contract_id: contract.id },
        });
        if (!error && data?.summary) setPlainSummary(String(data.summary));
      } catch (e) {
        console.warn("Plain-English summary unavailable (non-fatal):", e);
      }
    })();
  }, [contract, summaryRequested]);
