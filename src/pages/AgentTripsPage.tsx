...
export default function AgentTripsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (error) {
        console.error("Error loading agent trips:", error);
      } else {
        setTrips((data ?? []) as AgentTrip[]);
      }
      setLoading(false);
    })();
              ...
              >
                ...
                <div className="p-5 flex-1 flex flex-col gap-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <Badge className={`${statusStyles[status] ?? statusStyles.draft} rounded-full text-[10px] uppercase tracking-wider px-2.5 py-0.5 hover:${statusStyles[status] ?? ""}`}>
