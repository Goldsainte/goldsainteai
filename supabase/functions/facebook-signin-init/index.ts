...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('🔵 FACEBOOK SIGNIN INIT v1.0 - Starting flow');

    // Get Facebook credentials from database or env
    ...
    if (!appId || !appSecret) {
      ...
      return new Response(
        JSON.stringify({ error: 'Facebook Sign-In not configured' }),
        { status: 500, headers: { ...corsHeaders(req), 'Content-Type': 'application/json' } }
      );
    }

    console.log('Facebook credentials found, generating state...');
    ...
    console.log('Redirecting to Facebook OAuth with state cookie set');

    // Set HttpOnly cookie and redirect to Facebook (302)
    return new Response(null, {
      status: 302,
      headers: {
        'Location': authUrl.toString(),
        'Set-Cookie': `facebook_state=${state}; HttpOnly; Secure; SameSite=None; Path=/; Max-Age=600`,
        ...corsHeaders(req)
      }
    });
  } catch (error) {
    console.error('Error in facebook-signin-init:', error);
    return new Response(
