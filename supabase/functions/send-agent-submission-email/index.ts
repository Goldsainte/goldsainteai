...
const STRINGS: { en: S } & Partial<Record<EmailLang, S>> = {
  ...
  ar: { subject: (t) => `إعلان رحلتك قيد المراجعة \u2014 ${t}`, yourTripSubj: 'رحلتك', h1: 'أُرسلت الرحلة للمراجعة', hi: (n) => `مرحباً ${n}،`, thereFallback: '', received: (t) => `استلمنا إعلانك عن ${t} وهو الآن قيد المراجعة. يراجع فريقنا الإعلانات الجديدة عادة خلال 24\u201348 ساعة.`, yourTripBody: 'رحلتك', approval: 'ستصلك رسالة أخرى فور اعتماد إعلانك ونشره في السوق.', btnListings: 'اعرض إعلاناتي', questions: 'أسئلة؟ رد على هذه الرسالة أو زر مركز المساعدة.', footer: '\u00A9 2026 Goldsainte. سوق السفر الاجتماعي.' },
  ...
  ko: { subject: (t) => `여행 리스팅이 검토 중입니다 \u2014 ${t}`, yourTripSubj: '나의 여행', h1: '여행이 검토를 위해 제출되었습니다', hi: (n) => `안녕하세요 ${n}님,`, thereFallback: '', received: (t) => `${t} 리스팅이 접수되어 현재 검토 중입니다. 새 리스팅은 보통 24\u201348시간 안에 검토됩니다.`, yourTripBody: '나의 여행', approval: '리스팅이 승인되어 마켓플레이스에 게시되는 즉시 메일로 알려드립니다.', btnListings: '내 리스팅 보기', questions: '질문이 있으면 이 메일에 회신하거나 헬프 센터를 방문하세요.', footer: '\u00A9 2026 Goldsainte. 여행을 위한 소셜 마켓플레이스.' },
  zh: { subject: (t) => `你的旅程发布正在审核 \u2014 ${t}`, yourTripSubj: '你的旅程', h1: '旅程已提交审核', hi: (n) => `你好，${n}：`, thereFallback: '', received: (t) => `我们已收到你为 ${t} 提交的发布，正在审核中。团队通常在 24\u201348 小时内完成新发布的审核。`, yourTripBody: '你的旅程', approval: '发布通过审核并在市场上线后，你将立即收到另一封邮件。', btnListings: '查看我的发布', questions: '有疑问？回复本邮件或访问帮助中心。', footer: '\u00A9 2026 Goldsainte. 旅行的社交市场。' },
};
