...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await supabaseClient.from('notifications').insert({
      ...
      message: `Your payment of ${currency} ${amount} for "${job.title}" has been successfully processed through Stripe.`,
    ...
    if (agent?.user_id) {
      await supabaseClient.from('notifications').insert({
        ...
        message: `Payment of ${currency} ${amount} received for "${job.title}", paid directly to your Stripe account.`,
        entity_type: 'marketplace_job',
        entity_id: jobId,
